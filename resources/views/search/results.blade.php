...
@section('title', 'Pencarian - SiCAIR')
...
<nav class="sc-breadcrumb" aria-label="Breadcrumb">
    ...
    <span class="sc-breadcrumb-current">Hasil Pencarian</span>
</nav>

<div class="sc-page-header">
    <h2 class="sc-page-title mb-2">Pencarian</h2>
    <form method="GET" action="{{ route('search') }}" class="d-flex gap-2" style="max-width: 480px;">
        <div class="input-group">
            <span class="input-group-text" style="border-radius: 10px 0 0 10px; border: 2px solid var(--sc-border);">
                <i class="ti ti-search text-muted"></i>
            </span>
            <input type="text" name="q" value="{{ $query }}"
                   class="form-control"
                   placeholder="Cari nama pegawai, NIP, jenis cuti..."
                   style="border-radius: 0 10px 10px 0; border: 2px solid var(--sc-border); border-left: none;">
        </div>
        <button type="submit" class="btn sc-btn-primary" style="border-radius: 10px; white-space: nowrap;">
            Cari
        </button>
    </form>
</div>
        ...
        <div class="card sc-card">
            <div class="card-header">
                <h3 class="card-title mb-0">
                    <i class="ti ti-users me-2" style="color: var(--sc-primary);"></i>
                    Pegawai
                    <span class="badge ms-2" style="background:var(--sc-primary-light);color:var(--sc-primary);font-size:0.72rem;">{{ $pegawai->count() }}</span>
                </h3>
            </div>
            <div class="card-body p-0">
                @forelse($pegawai as $p)
                <div class="d-flex align-items-center gap-3 px-4 py-3 {{ !$loop->last ? 'border-bottom' : '' }}">
                    <div class="sc-user-avatar sc-user-avatar-sm" style="width:38px;height:38px;font-size:0.85rem;flex-shrink:0;background:var(--sc-primary);color:#fff;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:700;">
        ...
        <div class="card sc-card">
            <div class="card-header">
                <h3 class="card-title mb-0">
                    ...
                    Pengajuan Cuti
                    <span class="badge ms-2" style="background:var(--sc-primary-light);color:var(--sc-primary);font-size:0.72rem;">{{ $leaves->count() }}</span>
                </h3>
            </div>
            <div class="card-body p-0">
                ...
                <div class="d-flex align-items-center gap-3 px-4 py-3 {{ !$loop->last ? 'border-bottom' : '' }}">
                    <div class="flex-fill min-w-0">
                        ...
                        <div style="font-size:0.8rem;color:var(--sc-primary);">{{ $leave->type_label }}</div>
                    ...
                    @endphp
                    <span class="sc-badge {{ $badgeClass }}" style="white-space:nowrap;">{{ $leave->status_label }}</span>
