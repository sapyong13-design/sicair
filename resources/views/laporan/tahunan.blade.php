...
@section('title', 'Laporan Rekapitulasi Tahunan - SiHEALING')
...
<nav class="sh-breadcrumb" aria-label="Breadcrumb">
    ...
    <span class="sh-breadcrumb-current">Laporan Tahunan</span>
</nav>

<div class="sh-page-header">
    <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap">
        <div>
            <h2 class="sh-page-title mb-0">Laporan Rekapitulasi Cuti Tahunan</h2>
        ...
        <div class="d-flex gap-2">
            <a href="{{ route('laporan.tahunan.export', array_filter(['year' => $year, 'unit_kerja' => $unitKerja])) }}"
               class="btn sh-btn-primary" style="border-radius: 10px; font-size: 0.85rem;">
...
<div class="card sh-card">
    <div class="card-body p-0">
        ...
        <div class="table-responsive">
            <table class="table sh-table mb-0" style="font-size: 0.82rem; white-space: nowrap;">
                <thead>
                    <tr>
                        ...
                        @foreach($leaveTypes as $type)
                            <td class="text-center">
                                @if(isset($userPivot[$type]) && $userPivot[$type] > 0)
                                    <span style="color: var(--sh-primary); font-weight: 600;">{{ $userPivot[$type] }}</span>
                                @else
                                    <span class="text-muted">-</span>
                                @endif
                            </td>
                        @endforeach
                        <td class="text-center fw-bold" style="color: var(--sh-primary);">
                            {{ $rowTotal ?: '-' }}
                        </td>
                    </tr>
                    @endif
                ...
                <tfoot>
                    <tr class="table-light">
                        ...
                        @foreach($leaveTypes as $type)
                            ...
                            <td class="text-center fw-bold" style="color: var(--sh-primary);">{{ $colTotal ?: '-' }}</td>
                        @endforeach
                        @php $grandTotal = collect($pivot)->map(fn($row) => array_sum($row))->sum(); @endphp
                        <td class="text-center fw-bold" style="color: var(--sh-primary);">{{ $grandTotal }}</td>
                    </tr>
                </tfoot>
            </table>
        </div>
