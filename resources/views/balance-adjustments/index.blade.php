    ...
    <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
        <div>
            <h2 class="sc-page-title mb-0">
                <i class="ti ti-adjustments me-2" style="color: var(--sc-primary);"></i>
                Perubahan Saldo Cuti
            </h2>
            <div class="text-muted" style="font-size: 0.85rem;">Riwayat penyesuaian saldo cuti pegawai</div>
        </div>
        <a href="{{ route('pegawai.index') }}" class="btn sc-btn-primary">
    ...
    <div class="card-body p-3">
        <form method="GET">
            <div class="row g-2 align-items-end">
                ...
                <div class="col-md-2">
                    <label class="form-label" style="font-size: 0.78rem; font-weight: 600; color: var(--sc-text-muted); text-transform: uppercase; letter-spacing: 0.5px;">Tahun</label>
                    <select name="year" class="form-select" style="border-radius: 10px; border: 2px solid var(--sc-border); height: 42px;">
                ...
                <div class="col-md-4">
                    <label class="form-label" style="font-size: 0.78rem; font-weight: 600; color: var(--sc-text-muted); text-transform: uppercase; letter-spacing: 0.5px;">Cari Pegawai</label>
            ...
            <table class="table sc-table mb-0">
                ...
                <tbody>
                    @forelse($adjustments as $adjustment)
                        <tr>
                            <td>
                                <div class="fw-semibold" style="color: var(--sc-text);">{{ $adjustment->user->name }}</div>
                                <div class="text-muted" style="font-size: 0.8rem;">{{ $adjustment->user->email }}</div>
                            </td>
                            <td>
                                <span class="fw-semibold" style="color: var(--sc-text);">{{ $adjustment->year }}</span>
                            </td>
                            <td>
                                @switch($adjustment->type)
                                    @case('addition')
                                        <span class="sc-badge sc-badge-approved">
                                    ...
                                    @case('deduction')
                                        ...
                                        @break
                                    @case('correction')
                                        <span class="sc-badge" style="background: #e0f2fe; color: #0369a1;">
                            ...
                            <td>
                                ...
                                @endphp
                                <span class="sc-badge {{ $adjStatusClass }}">
